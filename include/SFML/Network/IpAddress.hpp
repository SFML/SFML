...
public:
    ...
    ///
    /// By default a provider hosted by SFML is used.
    ///
    /// When specifying a custom provider URL, the provider has to
    /// respond to an HTTP GET request with a body containing only
    /// the public IP address string in dotted-decimal notation in
    /// the case of an IPv4 address or internet standard notation
    /// in the case of an IPv6 address.
    ///
    /// Currently, SFML's provider does not support IPv6. In order
    /// to get the computer's public IPv6 address a provider like
    /// http://v6.ipinfo.io/ip can be used instead.
    ///
    /// Specifying a protocol (http:// or https://) is optional.
    /// Because the only information returned by the provider
    /// should be the public address which is already visible
    /// inside the IP packets, using HTTPS does not provide any
    /// advantages over HTTP in this scenario. If only one of the
    /// two protocols is provided then it has to be chosen. If no
    /// protocol is specified HTTP is automatically used.
    ///
    /// \param timeout  Maximum time to wait
    /// \param type     The type of public address to get
    /// \param provider Provider to use to determine the public IP address, defaults to SFML's provider
    ///
    ...
    [[nodiscard]] static std::optional<IpAddress> getPublicAddress(
        ...
        Type               type     = Type::IpV4,
        const std::string& provider = "http://www.sfml-dev.org/ip-provider.php");
...
/// auto a9  = sf::IpAddress::getPublicAddress(sf::IpAddress::Type::IpV4);                                              // my IPv4 address on the internet
/// auto a10 = sf::IpAddress::getPublicAddress(sf::IpAddress::Type::IpV6);                                              // my IPv6 address on the internet
/// \endcode
///
