...
T swapEndian(T value)
{
    ...
    std::array<std::uint8_t, sizeof(T)> rawBytes;

    auto* valueBytes = reinterpret_cast<std::uint8_t*>(&value);
    std::memcpy(rawBytes.data(), valueBytes, sizeof(T));

    std::array<std::uint8_t, sizeof(T)> swapped;
    std::reverse_copy(rawBytes.begin(), rawBytes.end(), swapped.begin());
    std::memcpy(valueBytes, swapped.data(), sizeof(T));

    return *reinterpret_cast<T*>(valueBytes);
}
} // namespace
