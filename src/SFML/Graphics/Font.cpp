...
private:
    ...
    struct Deleter
    {
        void operator()(FT_Library library) { FT_Done_FreeType(library); }
        void operator()(FT_Face face)       { FT_Done_Face(face); }
        void operator()(FT_Stroker stroker) { FT_Stroker_Done(stroker); }
    };

public:
    static_assert(std::is_pointer_v<FT_Library>);
    static_assert(std::is_pointer_v<FT_Face>);
    static_assert(std::is_pointer_v<FT_Stroker>);
...
bool Font::loadFromFile(const std::string& filename)
{
    ...
    auto font = std::make_unique<priv::FontHandles>();
