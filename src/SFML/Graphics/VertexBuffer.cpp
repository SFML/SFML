...
bool VertexBuffer::create(std::size_t vertexCount)
{
    ...
    glCheck(GLEXT_glBufferData(GLEXT_GL_ARRAY_BUFFER, static_cast<GLsizeiptrARB>(sizeof(Vertex) * vertexCount), 0, usageToGlEnum(m_usage)));
...
bool VertexBuffer::update(const VertexBuffer& vertexBuffer)
{
    ...
    glCheck(GLEXT_glBindBuffer(GLEXT_GL_ARRAY_BUFFER, m_buffer));
    glCheck(GLEXT_glBufferData(GLEXT_GL_ARRAY_BUFFER, static_cast<GLsizeiptrARB>(sizeof(Vertex) * vertexBuffer.m_size), 0, usageToGlEnum(m_usage)));
