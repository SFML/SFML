...
////////////////////////////////////////////////////////////
std::optional<IpAddress> IpAddress::getPublicAddress(Time timeout, Type type, const std::string& provider)
{
    // The trick here is more complicated, because the only way
    // to get our public IP address is to get it from a distant computer.
    // Here we get the web page from e.g. http://www.sfml-dev.org/ip-provider.php
    // and parse the result to extract our IP address
    // (not very hard: the web page contains only our IP address).

    auto uriStart = std::string::npos;

    if (toLower(provider.substr(0, 7)) == "http://")
    {
        uriStart = provider.find('/', 7);
    }
    else if (toLower(provider.substr(0, 8)) == "https://")
    {
        uriStart = provider.find('/', 8);
    }
    else
    {
        uriStart = provider.find('/');
    }

    const Http          server(provider.substr(0, uriStart), 0, type != Type::IpV4);
    const Http::Request request((uriStart != std::string::npos) ? provider.substr(uriStart) : "/",
    ...
    {
        if (const auto address = IpAddress::fromString(page.getBody()); address)
        {
            // If the user explicitly requested a specific type of
            // address and we got the other type we return nullopt
        ...
        err() << "Failed to retrieve public address from external IP resolution server (Could not parse address \""
              << page.getBody() << "\")" << std::endl;
    }
    else
    {
