...
m_libraryName(libraryName), m_moduleHandle(LoadLibraryW(m_libraryName.toWideString().c_str()))
{
}
...
void LibraryLoaderImpl::freeLibrary()
{
    if (m_moduleHandle != nullptr)
    {
        if (!FreeLibrary(m_moduleHandle))
            sf::err() << "Failed to free library " << m_libraryName.toAnsiString() << std::endl;
...
bool LibraryLoaderImpl::getProcedureAddress(void*& procedureAddress, const sf::String& procedureName) const
{
    ...
    procedureAddress = GetProcAddress(m_moduleHandle, procedureName.toAnsiString().c_str());
