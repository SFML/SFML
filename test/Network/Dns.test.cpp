...
TEST_CASE("[Network] sf::Dns (IPv4)", runIpV4InternetTests())
{
    ...
    SECTION("www.sfml-dev.org")
    {
        ...
        CHECK(!addresses.front().toString().empty());
    }
...
// Advanced DNS queries are currently not supported on Android by the current implementation
TEST_CASE("[Network] sf::Dns (Advanced Queries)", runDnsTestsAdvanced())
{
    SECTION("MX records")
    {
        ...
        CHECK(!records.front().exchange.empty());
    ...
    SECTION("SRV records")
    {
        ...
        REQUIRE(!records.empty());
        CHECK(!records.front().target.empty());
