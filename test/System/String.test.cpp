...
TEST_CASE("[System] sf::String")
{
    ...
    SECTION("Construction")
    {
        ...
        {
            const sf::String string = U'\U0001F40C';
            CHECK(std::string(string) == "\0"s);
            CHECK(std::wstring(string) == select(L""s, L"\U0001F40C"s));
            CHECK(string.toAnsiString() == "\0"s);
            CHECK(string.toWideString() == select(L""s, L"\U0001F40C"s));
            CHECK(string.toUtf8() == sf::U8String{0xF0, 0x9F, 0x90, 0x8C});
            CHECK(string.toUtf16() == u"\U0001F40C"s);
            CHECK(string.toUtf32() == U"\U0001F40C"s);
            CHECK(string.getSize() == 1);
            CHECK(!string.isEmpty());
            CHECK(string.getData() != nullptr);
        }
        ...
        SECTION("UTF-32 C string constructor")
        {
            ...
            SECTION("Non-empty string")
            {
                const sf::String string = U"\U0001F40Crs";
                CHECK(std::string(string) == "\0rs"s);
                CHECK(std::wstring(string) == select(L"rs"s, L"\U0001F40Crs"s));
                CHECK(string.toAnsiString() == "\0rs"s);
                CHECK(string.toWideString() == select(L"rs"s, L"\U0001F40Crs"s));
                CHECK(string.toUtf8() == sf::U8String{0xF0, 0x9F, 0x90, 0x8C, 'r', 's'});
                CHECK(string.toUtf16() == u"\U0001F40Crs"s);
                CHECK(string.toUtf32() == U"\U0001F40Crs"s);
                CHECK(string.getSize() == 3);
                CHECK(!string.isEmpty());
                CHECK(string.getData() != nullptr);
            }
        ...
        {
            const sf::String string = U"tuv\U0001F40C"s;
            CHECK(std::string(string) == "tuv\0"s);
            CHECK(std::wstring(string) == select(L"tuv"s, L"tuv\U0001F40C"s));
            CHECK(string.toAnsiString() == "tuv\0"s);
            CHECK(string.toWideString() == select(L"tuv"s, L"tuv\U0001F40C"s));
            CHECK(string.toUtf8() == sf::U8String{'t', 'u', 'v', 0xF0, 0x9F, 0x90, 0x8C});
            CHECK(string.toUtf16() == u"tuv\U0001F40C"s);
            CHECK(string.toUtf32() == U"tuv\U0001F40C"s);
            CHECK(string.getSize() == 4);
            CHECK(!string.isEmpty());
            CHECK(string.getData() != nullptr);
        }
    ...
    SECTION("fromUtf32()")
    {
        constexpr std::array<char32_t, 4> characters{'w', U'\U0001F40C', 'y', 'z'};
        const sf::String                  string = sf::String::fromUtf32(characters.begin(), characters.end());
        CHECK(std::string(string) == "w\0yz"s);
        CHECK(std::wstring(string) == select(L"wyz"s, L"w\U0001F40Cyz"s));
        CHECK(string.toAnsiString() == "w\0yz"s);
        CHECK(string.toWideString() == select(L"wyz"s, L"w\U0001F40Cyz"s));
        CHECK(string.toUtf8() == sf::U8String{'w', 0xF0, 0x9F, 0x90, 0x8C, 'y', 'z'});
        CHECK(string.toUtf16() == u"w\U0001F40Cyz"s);
        CHECK(string.toUtf32() == U"w\U0001F40Cyz"s);
        CHECK(string.getSize() == 4);
        CHECK(!string.isEmpty());
        CHECK(string.getData() != nullptr);
    }
